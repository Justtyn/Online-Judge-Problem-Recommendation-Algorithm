...
attempt_stats["perseverance"] = attempt_stats["avg_attempts_per_problem"].apply(
    lambda x: min(1.0, math.log1p(x) / den if den > 0 else 0.0))
...
lang_pref = lang_tab.apply(
    lambda r: json.dumps({k: float(r.get(k, 0.0)) for k in lang_keys if r.get(k, 0.0) > 0}, ensure_ascii=False),
    axis=1).reset_index()
...
tag_tab = tag_counts.pivot_table(index="user_id", columns="tag", values="cnt", fill_value=0).reindex(columns=tag_vocab,
                                                                                                     fill_value=0)
tag_tab = tag_tab.div(tag_tab.sum(axis=1).replace(0, 1), axis=0)
tag_pref = tag_tab.apply(
    lambda r: json.dumps({k: float(r.get(k, 0.0)) for k in tag_vocab if r.get(k, 0.0) > 0}, ensure_ascii=False),
    axis=1).reset_index()
tag_pref.columns = ["user_id", "tag_pref"]

users = pd.DataFrame({"user_id": sorted(subs["user_id"].unique().tolist())})
out = users.merge(level, on="user_id", how="left").merge(attempt_stats[["user_id", "perseverance"]], on="user_id",
                                                         how="left") \

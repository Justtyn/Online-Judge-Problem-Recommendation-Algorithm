...
- [ ] 生成 `CleanData/students_derived.csv`
...
- [ ] `Reports/fig_level_hist.png`：用户 `level` 分布直方图
- [ ] `Reports/fig_perseverance_hist.png`：用户 `perseverance` 分布直方图
- [ ] `Reports/fig_lang_dist.png`：语言总体分布柱状图（Python/C/C++/JS/JAVA/GO 等）
- [ ] `Reports/fig_tag_dist.png`：标签总体分布柱状图（12 类题型占比）
- [ ] `Reports/fig_user_activity.png`：用户做题量/提交量分布（长尾）
...
- [ ] 生成 `FeatureData/train_samples.csv`
  - `y`：是否 AC（1/0）
  - `X`：建议包含
...
- [ ] `Reports/fig_difficulty_vs_ac.png`：`difficulty` vs AC 率（难度越高通过率应下降）
- [ ] `Reports/fig_attemptno_vs_ac.png`：`attempt_no` vs AC 率曲线
- [ ] `Reports/fig_tag_acrate.png`：各标签平均 AC 率柱状图
- [ ] `Reports/fig_lang_acrate.png`：各语言 AC 率柱状图
...
- [ ] Logistic Regression（主模型，输出可解释系数）
- [ ] Decision Tree（对比，输出特征重要性）
- [ ] SVM 或 KNN（二选一对比）

### 5.2 输出（必须）

- [ ] `Models/metrics.csv`：每个模型的 `Accuracy/Precision/Recall/F1`（train/test 都建议记录）
- [ ] （可选）`Models/model_*.pkl`：保存训练好的模型/预处理器（pipeline）

### 5.3 可视化（强烈建议）

- [ ] `Reports/fig_metrics_train_test.png`：训练集 vs 测试集 指标对比柱状图（判断过拟合）
- [ ] `Reports/fig_cm_logreg.png`、`Reports/fig_cm_tree.png`、`Reports/fig_cm_svm_or_knn.png`：混淆矩阵 heatmap
...
- [ ] `Reports/recommendations_topk.csv`
  - 至少包含 20–50 个用户示例
  - 字段建议：`student_id, problem_id, score(p_ac), difficulty, tags, reason(optional)`

### 6.2 评估指标（必须）

- [ ] 计算 `Hit@K` / `Precision@K`（K=1,3,5,10）
  - 评估集：时间切分后的测试窗口（用“后续是否 AC/是否尝试”做命中定义，按你报告口径固定）

### 6.3 可视化（建议）

- [ ] `Reports/fig_hitk_curve.png`：Hit@K 随 K 变化曲线
- [ ] `Reports/fig_reco_difficulty_hist.png`：用户案例推荐题难度分布（是否符合“成长型”）
- [ ] `Reports/fig_reco_coverage.png`：覆盖率/集中度（避免只推荐少数热门题）

验收点：
- [ ] 推荐只从“用户未 AC（或未做过）”题集中选
- [ ] 概率区间过滤与 Top-K 逻辑可复现（写成函数/脚本）

---

## 最终交付清单（写报告用）

- [ ] 所有 CSV：`CleanData/students_derived.csv`、`FeatureData/train_samples.csv`、`Models/metrics.csv`、`Reports/recommendations_topk.csv`

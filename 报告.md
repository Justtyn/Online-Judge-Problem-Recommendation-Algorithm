...
- 画像派生命令：`python 01_derive_students.py` → `CleanData/students_derived.csv`

## 2.4 特征工程

特征设计强调“可解释 + 可复现 + 无泄漏”。训练样本由 `python 02_build_features.py` 生成至 `FeatureData/train_samples.csv`，每行对应一次提交。
...
```bash
python Utils/validate_originaldata.py
python 01_derive_students.py
python 02_build_features.py
python 03_train_eval.py
python 04_recommend_eval.py
python 05_make_eda_plots.py
```
